...
export default async function editContactEntryEvent(event: Electron.IpcMainEvent, currentAndNew: CurrentAndNewContactEntries) {
    ...
    try {
        ...
        const oldContactEntryKey = ContactEntry.getContactEntryKey(currentAndNew.current);
        if ((newContactEntryKey in contactEntries) && (newContactEntryKey !== oldContactEntryKey)) throw new Error("The contact specified already exists elsewhere.");
